...
export default function Dashboard() {
  ...
  const runScan = useCallback(async () => {
    ...
    try {
      ...
      setTotalBought(prev => prev + (data.stats?.kjopt || 0));
      setTotalSold(prev => prev + (data.stats?.solgt || 0));
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-card border border-border rounded-lg p-6">
            <h3 className="font-semibold mb-4">Aktive Signaler</h3>
            {signals.length > 0 ? (
                ...
                {signals.map((signal, i) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-medium">{signal.amount}x {signal.ticker}</span>
                    </div>
            ...
            {failedTrades.length > 0 && (
              <div className="mt-4 pt-4 border-t border-border">
                <p className="text-xs text-red-400">
                  {failedTrades.length} feilet: {failedTrades.map(t => t.ticker).join(', ')}
                </p>
              </div>
            )}
          </div>
