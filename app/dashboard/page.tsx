...
interface PerformancePayload {
  current: { totalValue: number; pnl: number; pnlPercent: number; initialValue: number };
  session: { peak: number; maxDrawdown: number };
  chartData: Array<{ time: string; value: number; pnl: number; pnlPercent: number }>;
}
...
export default function DashboardPage() {
  ...
  // Live ticker — refresh every 3 s while connected.
  useEffect(() => {
      ...
      <main className="canvas">
        <div className="col-l">
          <div className="panel">
            ...
            />
            <ReturnsChart points={equityPoints} />
          </div>
          <StatGrid
            lang={lang}
            startVal={startVal}
        ...
        <div className="col-r">
          <SystemStatus lang={lang} scanCount={scanCount} />
          <KillSwitch
            lang={lang}
            onKill={() =>
              toast.error(lang === 'no' ? 'NØDSTOPP aktivert' : 'Emergency stop activated')
            }
          />
